...
LastEditTime: 2025-08-12 17:38:54
FilePath: /rknn-llm/examples/Qwen2.5-VL-instruct-3b/export/export_vision_onnx.py
...
def packing_small_onnx_files(onnx_path):
    ...
    save_model(load_model(onnx_path), 
            ...
            # location=str(onnx_path.parent / f"{onnx_path.name}.extra_file"),
            save_as_external_data=True,
            all_tensors_to_one_file=True,
            size_threshold=1024,
            convert_attribute=True)
    print(f"save external data to {onnx_path}.extra_file done!")
    onnx_path = Path(onnx_path)
    onnx_dir = onnx_path.parent
    for file in onnx_dir.iterdir():
        if file.name.startswith("_visual_") or file.name.startswith("visual."):
            print(f"remove {file}")
            file.unlink()
...
argparse.add_argument('--savepath', type=str, default='onnx_model/qwen2.5_vl_3b_vision.onnx', help='save path', required=False)
...
if step == 1:
    ...
    torch.onnx.export(model, pixel_values, args.savepath, opset_version=15) # 这里需要指定opset<18

    # Packing small ONNX files into an extra file
    packing_small_onnx_files(args.savepath)
